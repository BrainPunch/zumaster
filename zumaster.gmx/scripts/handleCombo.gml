...
ds_queue_enqueue(queue, source.gridX, source.gridY);
while (!ds_queue_empty(queue)) {
    ...
    ds_queue_enqueue(queue, ball.gridX-1, ball.gridY);
    ds_queue_enqueue(queue, ball.gridX+1, ball.gridY);
    ds_queue_enqueue(queue, ball.gridX, ball.gridY-1);
    ds_queue_enqueue(queue, ball.gridX, ball.gridY+1);
}
ds_queue_destroy(queue);

if (ds_list_size(found) >= 2) {
